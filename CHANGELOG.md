...
### Corrigido
* **Resolução Dinâmica do Executável FFmpeg (`🔒 FIX`):** Corrigido desvio no qual comandos chamavam o `ffmpeg` genérico do sistema (`/usr/bin/ffmpeg`) em vez do binário customizado com suporte a NVENC embutido em `assets/bin/` (Flatpak/Windows). O `build_ffmpeg_command` agora prioriza `self.ffmpeg_bin` para opções genéricas/padrão, `main.py` injeta `assets/bin` no `PATH` em todas as plataformas (Linux/Windows) e o campo `entry_ffmpeg_path` da GUI inicializa e restaura com o caminho real resolvido.
* **Eliminação de `-cq 18` Conflitante no NVENC (`🔒 FIX`):** Removido parâmetro estático `-cq 18` do preset NVENC que causava colisão com o valor de CRF configurado na interface (`-cq 23`), unificando o controle de qualidade sob a governança única do seletor de CRF/Bitrate sem flags redundantes ou duplicadas.
* **Resolução Automática de Dependências do Flatpak no `auto_build.sh` (`🔒 FIX`):** O script de build agora instala explicitamente tanto a Platform (`org.kde.Platform/x86_64/6.11`) quanto o SDK (`org.kde.Sdk/x86_64/6.11`) no perfil do usuário de modo não interativo (`--user --noninteractive -y`), eliminando falhas de runtime ausente durante a execução do `flatpak-builder`.
* **Migração da KDE Platform para 6.11 no Flatpak (`🔒 FIX`):** Atualizada a base do runtime e do SDK de `6.9` para `6.11` (`org.kde.Platform 6.11` e `org.kde.Sdk 6.11`), versão estável oficial aceita no Flathub baseada no Freedesktop SDK 25.08. Removida a declaração manual da extensão legada `org.freedesktop.Platform.ffmpeg-full:24.08`, integrando a aplicação à nova arquitetura de codecs do Flathub provida automaticamente pela extensão nativa `org.freedesktop.Platform.codecs-extra:25.08-extra`.
* **Falha de Compilação do FFmpeg no Flatpak com Texinfo 7.1+ (`🔒 FIX`):** Adicionada a flag `--disable-doc` nas opções de configuração do módulo `ffmpeg` no manifesto `build_scripts/com.github.vagnarok.lyra.yml`. A geração de documentação HTML invocava o script Perl `./doc/t2h.pm`, que falhava com `Can't locate object method 'gdt' via package 'Texinfo::Convert::HTML'` devido a alterações de API no Texinfo 7.1+ presente no KDE SDK 6.11 / Freedesktop SDK 25.08.
* **Crash Silencioso no Flatpak ao Iniciar / Aborto SIGABRT no libmpv (`🔒 FIX`):** Ao executar o pacote Flatpak (`flatpak run com.github.vagnarok.lyra`), o aplicativo fechava imediatamente com código de erro 134 devido a uma falha na asserção `assert(h->canary == CANARY)` do alocador `ta/ta.c` do `libmpv` (0.35.1). A falha ocorria porque a união `union m_option_value val = {0};` em `m_config_set_option_cli` inicializava somente o primeiro membro de 1 byte (`bool`), deixando ponteiros na stack com lixo de memória que eram indevidamente liberados pelo `copy_str`. O problema foi corrigido aplicando patch em `build_scripts/mpv-ffmpeg7.patch` com `memset(&val, 0, sizeof(val))` e habilitando no manifesto `build_scripts/com.github.vagnarok.lyra.yml` as opções de release (`-Dbuildtype=release`, `-Ddebug=false` e `cflags: "-O2 -DNDEBUG -Wno-error=incompatible-pointer-types"`), desativando asserções de depuração em produção conforme as diretrizes do Flathub.
