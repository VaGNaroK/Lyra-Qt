...
* **Resolução Automática de Dependências do Flatpak no `auto_build.sh` (`🔒 FIX`):** O script de build agora instala explicitamente tanto a Platform (`org.kde.Platform/x86_64/6.9`) quanto o SDK (`org.kde.Sdk/x86_64/6.9`) e a extensão de codecs (`org.freedesktop.Platform.ffmpeg-full:24.08`) no perfil do usuário de modo não interativo (`--user --noninteractive -y`), eliminando falhas de runtime ausente durante a execução do `flatpak-builder`.
* **Aviso de Fim de Vida (EOL) do Runtime KDE no Flatpak (`🔒 FIX`):** Migrada a versão base do runtime e do SDK de `6.8` para `6.9` (`org.kde.Platform 6.9` e `org.kde.Sdk 6.9`) no manifesto `build_scripts/com.github.vagnarok.lyra.yml` e no script `auto_build.sh`, eliminando o alerta *"este pacote está em fim de vida e não receberá mais atualizações de segurança"* emitido pelo Flathub e preservando 100% de compatibilidade com a extensão de codecs `org.freedesktop.Platform.ffmpeg-full:24.08`.
